...
while(true)
{
    ...
    switch(input)
    {
      case 1:
             ...
             if(DateTime.TryParse(dataScadenzastringa, out DateTime dataScadenza))
              {
                listaProdotti[prodotto] = dataScadenza.ToString("dd/MM/yyyy");
              }
             else
             {
      ...
      case 2:
            ...
            {
                Console.WriteLine($"Prodotto : {kvp.Key}, Scadenza: {kvp.Value}");
            }
    ...
    /*Console.WriteLine("Inserisci il nome di un prodotto da aggiungere alla lista:");
    string prodotto = Console.ReadLine();
    Console.WriteLine("Inserisci la data di un prodotto da aggiungere alla lista: gg/mm/aa");
    string inputData   = Console.ReadLine();
    listaProdotti.Add(prodotto, inputData);
    DateTime dataOdierna = DateTime.Today;

    if(DateTime.TryParse(inputData, out DateTime dataScadenza))
    {
    TimeSpan giorniMancanti = dataScadenza - dataOdierna;
    string stato;
    ...
    {
        Console.WriteLine("Conversione non riuscita, inserire un formato valido.");
    }
...
foreach(var kvp in listaProdotti)
{
 Console.WriteLine($"Nome prodotto: {kvp.Key}, data: {kvp.Value}");
}
*/
